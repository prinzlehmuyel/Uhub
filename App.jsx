...
function Dropdown({ label, value, onChange, options, required, placeholder }) {
  const isEmpty = !options || options.length === 0;
  return (
    <div style={S.formGroup}>
      {label && <label style={S.label}>{label}{required && <span style={{ color: RED }}> *</span>}</label>}
      <select value={value} onChange={e => onChange(e.target.value)} style={S.select} required={required} disabled={isEmpty}>
...
// so an announcement targeting a course could otherwise silently miss
// students whose course code differs only in spacing/case.
