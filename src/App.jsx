...
function Dashboard({ user, userData, announcements = [], onOpenAnnouncements, onNavigate,
  unreadAnnouncements = 0, unreadAssignments = 0, unreadExams = 0, pendingTasks = 0 }) {
  ...
  return (
    <div style={S.page}>
      ...
      {/* Notification summary — announcements/assignments/exams show UNREAD
          counts (cleared by opening that page); personal tasks shows a
          plain incomplete-count instead, since completion is self-reported. */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 10, marginBottom: 16 }}>
        ...
        <div onClick={() => onNavigate?.("assignments")} style={{ ...S.stat, cursor: "pointer" }}>
          <div style={{ ...S.statNum, color: unreadAssignments > 0 ? RED : TEXT }}>{unreadAssignments}</div>
          <div style={S.statLabel}>New Assignments</div>
        </div>
        <div onClick={() => onNavigate?.("exams")} style={{ ...S.stat, cursor: "pointer" }}>
          <div style={{ ...S.statNum, color: unreadExams > 0 ? RED : TEXT }}>{unreadExams}</div>
          <div style={S.statLabel}>Upcoming Exams</div>
        </div>
        <div onClick={() => onNavigate?.("assignments")} style={{ ...S.stat, cursor: "pointer" }}>
          <div style={S.statNum}>{pendingTasks}</div>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const collections = ["courses", "assignments", "notes", "flashcards", "gpaRecords", "examDates", "studyPlans", "savedResources"];
  ...
  // unread until the student's first visit to that page.
  ...
  // Personal tasks are a workload count, not an unread count — completion
  // is self-reported, so "unread" doesn't apply the same way.
  const pendingTasks = (userData.assignments || []).filter(a => !a.done).length;
  ...
  const pages = {
    dashboard:   <Dashboard   user={user}  userData={userData} announcements={visibleAnnouncements}
                    ...
                    unreadAnnouncements={unreadAnnouncements} unreadAssignments={unreadAssignments}
                    unreadExams={unreadExams} pendingTasks={pendingTasks} />,
