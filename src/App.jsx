...
// ─── CONSTANTS ────────────────────────────────────────────────────────────────
// Faculties, departments, and levels are managed by the Admin Dashboard in
// Firestore ("faculties" / "departments" / "levels" collections). These
// arrays are only a FALLBACK — shown for an instant before the live Firestore
// data loads, or if an admin ever deletes every entry. Once Firestore has
// data, it's used exclusively, so anything admin adds/edits/deletes is
...
const DEFAULT_LEVELS = ["100 Level","200 Level","300 Level","400 Level","500 Level","600 Level","Postgraduate"];
// Kept as a plain alias — used elsewhere in the app (course-level tagging in
// the Courses/GPA calculators) where a live Firestore lookup isn't needed.
...
function useLiveFaculties() {
  ...
  const names = faculties.length > 0 ? faculties.map(f => f.name) : DEFAULT_FACULTIES;
...
function useLiveLevels() {
  ...
  const names = levels.length > 0 ? levels.map(l => l.name) : DEFAULT_LEVELS;
  return names;
}
...
function Courses({ uid, userData, user }) {
  ...
  const [level, setLevel]     = useState(user?.profile?.level || LEVELS[0]);
  ...
  const save = async () => {
    ...
    try {
      await addItem(uid, "courses", { code: code.toUpperCase(), title, units: Number(units), level, semester });
  ...
  // Group by level (in LEVELS order), then by semester within each level.
  // Courses saved before the level field existed fall back to "Unspecified Level".
  const levelsPresent = LEVELS.filter(l => courses.some(c => (c.level || "Unspecified Level") === l));
  const otherLevels = [...new Set(courses.map(c => c.level || "Unspecified Level"))].filter(l => !LEVELS.includes(l));
...
function canViewAnnouncement(a, viewerProfile, myCourses, viewerUid) {
  ...
  switch (a.audienceType) {
    ...
    case "student":    return a.audienceStudentUid ? a.audienceStudentUid === viewerUid
                                                    : viewerProfile.email === a.audienceValue;
    ...
    case "course":     return (myCourses || []).some(c => c.code === a.audienceValue);
    default:           return false;
  }
}
      ...
      <div style={{ ...S.card, marginTop: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 8 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <Dropdown label="" value={level} onChange={v => { setLevel(v); setGrades({}); }} options={LEVELS} />
