...
export default function App() {
  ...
  // the bottom-nav badges and the announcements panel bell — they clear
  // the moment the student opens that page (see goToTab/openAnnouncements
  // below) and are otherwise unrelated to whether the item is still
  // relevant.
  ...
  // own personal lectures, filtered to today's day of week and sorted by
  // start time, for the Dashboard's "Today's Classes" card.
  const todayDayName = new Date().toLocaleDateString("en-US", { weekday: "long" });
  const todayLectures = [
    ...visibleAdminLectures.filter(l => l.dayOfWeek === todayDayName).map(l => ({ ...l, source: "admin" })),
    ...(userData.lectures || []).filter(l => l.dayOfWeek === todayDayName).map(l => ({ ...l, source: "personal" })),
  ].sort((a, b) => (a.startTime || "").localeCompare(b.startTime || ""));

  // Every place that navigates to a notification-bearing page marks that
  // type as read at the same time — one call, and the badge for that type
  // clears immediately (readStatus updates live via the listener above).
  ...
  return (
    <div style={S.app}>
      ...
      <nav style={S.bottomNav}>
        {TABS.map(t => {
          const badgeCount = t.id === "assignments" ? unreadAssignments : t.id === "exams" ? unreadExams : 0;
          return (
