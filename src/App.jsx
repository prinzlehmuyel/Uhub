...
// ─── ASSIGNMENTS ──────────────────────────────────────────────────────────────
function Assignments({ uid, userData, user }) {
  ...
  // ── Assignments issued by admin (shared, top-level `assignments`
  // collection — separate from this student's own private list above).
  // Course-targeted ones are matched via normalized course code against
  // this student's own course list, so adding PHT401 (in any spacing/case)
  // surfaces any assignment admin targeted at that course automatically.
  const [adminAssignments, setAdminAssignments] = useState([]);
  useEffect(() => listenToTopCollection("assignments", setAdminAssignments), []);
  const profile = user?.profile || {};
  const visibleAdminAssignments = adminAssignments
    .filter(a => canViewAssignment(a, profile, userData.courses, uid))
...
export default function App() {
  ...
  const pages = {
    ...
    assignments: <Assignments uid={user.uid} userData={userData} user={user} />,
